...
namespace Viry3D
{
    ...
    {
#if VR_VULKAN
        m_vertex_buffer = Display::Instance()->CreateBuffer(&vertices[0], vertices.SizeInBytes(), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, false, VK_FORMAT_UNDEFINED);
        m_index_buffer = Display::Instance()->CreateBuffer(&indices[0], indices.SizeInBytes(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, false, VK_FORMAT_UNDEFINED);
#elif VR_GLES
