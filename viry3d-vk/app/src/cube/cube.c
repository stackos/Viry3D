...
#ifdef VR_WINDOWS
static char *tex_files[] = { "Assets/cube/lunarg.ppm" };
#else
...
static void demo_create_window(struct demo *demo) {
    ...
    demo->window = CreateWindowEx(0,
        ...
        (rect.bottom - rect.top - height) / 2,            // x/y coords
        width,  // width
        height,  // height
...
static void demo_init(struct demo *demo) {
    ...
    int width = demo->width;
    int height = demo->height;
    ...
    mat4x4_perspective(demo->projection_matrix, (float)degreesToRadians(45.0f), demo->width / (float) demo->height, 0.1f, 100.0f);
